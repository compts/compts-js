...
function domCSS(id,d){
	var elem_str_class="";
	var get_attr=(_ct.has(getDomAttr(id,['style']).style))?getDomAttr(id,['style']).style:"";
...
function url_filter_restriction(url){
	var ur1=url.toLowerCase().replace(/^(https:)/g,"");
	return ur1;
}
function checkurlvalid(s){
	if(/^(HTTP|HTTPS|SFTP|FTP|TCP|\/{2})/.test(s.toUpperCase())){
	
			return url_filter_restriction(s);
		}
	else{
		return s;
		}
...
function checkurlvalid_ajax(s){
		...
		var url=root.location.host;
			return url_filter_restriction(proto+"//"+url+"/"+s);
			}else{
				var url_m=ps_glbl.config.url;

				return url_filter_restriction(url_m+""+s);
			}	
			}
		
}	
function dom_offset(_el){
			var target = _el,
...
function js_event_type(xhr,htype,func,jsons,func_type){
				var retrn={};
...
function js_json_delimiters(jsn){
...
function get_element_exist_attr(res){
	var attr_elem={};
...
function ajaxHttpLoader(main_jsn,xdr,method,url,parameter,dataform){

			

	try{
		var js_url=checkurlvalid_ajax(url);
		try{
...
function isWhereElement(tar_m,ar){
	var node=[];
...
function DomEventIniate(main,spltt,fn,bools){
	...
	for(var v=0;v<spltt.length;v++){
		...
		(function(main,m1,m2,m3,func){
			...
			eventLoop(main,m1,m2,m3,func,bools);
...
function eventLoop(elthis,c1,c2,c3,func,act_bool){
...
function elemDelegateEvent(agentype,agentversion,elem,evnt,func,bools){
...
function find_element(tar_m_sub,ar,bool){
	...
	for( var ni in node ){
	   for( var ti in tar_m ){
		...
		if(cek>0) find_element(cev,ar,bool);
		});
